...
Copyright: &copy; 2024, Luke Davis and NV Access Limited, all rights reserved.

## Introduction

For various reasons, you may not be able to, or may not wish to, construct the [recommended build environment](./buildingNVDA.md) for NVDA.
...
Subsections are indicated at heading level three, and subsidiary items are given heading level five.
...
It is assumed that you have already read the [NVDA] page on [Contributing], in particular the section on the [GitHub process](https://github.com/nvaccess/nvda/wiki/Contributing#github-process), as you will need to have an [NVDA] fork already in place on your development computer.

Furthermore, you should already be generally familiar with [git], [GitHub], and how to work with repositories.
This article won't go into any of these basic areas in any detail, and will only summarize topics the reasonably experienced developer is likely to understand already.
...
It is discussed in the [Self-signed Build](./selfSignedBuild.md) document, but not in the context of building forks on [Appveyor].
...
For the rest of this article, we will assume you have called it "nvda".
...
Unfortunately, since NV Access already uses Appveyor for official builds, it would be difficult to keep your build file in the root directory of your NVDA fork.This is because you will need to permanently change a few items in that file, and you wouldn't want that private build file "contaminating" your pull requests or diffs.
...
You may also use a [GitHub] gist, which is described below.
...
*Note, that [Appveyor] themselves, provide instructions for using a gist in this way.
However, they assume you will be using the web to manage and update your gist, and that process was not perfectly accessible as of 2023.
Never the less, those instructions can be found [here](https://www.appveyor.com/docs/build-configuration/#alternative-yaml-file-location), if you want to read them.*
...
2. Change the following settings:
    ...
    * Find the checkbox for "Rolling builds".
        You don't have to, but probably do want to select this; read more about [rolling builds here](https://www.appveyor.com/docs/build-configuration/#rolling-builds).
...
That is basically the opposite of what we want in a non-NV Access fork.
...
But, you don't want your Appveyor worker to build these, both because NV Access's setup is going to build them; and because if you use any notification section (see later), you don't want NV Access getting notified and bothered by your fork's CI activity.
...
Therefore, these sections must be "turned off".
...
If you can't find a "`CANCEL BUILD`" button for the in-progress build, try using the "`Current build`" link under the project name heading.
If something still doesn't appear to be working correctly, try refreshing the project page.
...
We do not anticipate that most users will need these sections.
However, if you want to work on NVDA's build scripts or build process directly, you may find the following of use.

### Advanced topic: a build control branch

While the gist or personal website methods for storing `appveyor.yml` are likely sufficient for most, developers who wish to change NVDA's build scripts or the Appveyor process itself, will need a different approach.
...
`testCode` is checked out into your Appveyor build environment.
