...
class GlobalCommands(ScriptableObject):
	...
	@script(
		description=_(
			...
			"Speak the destination URL for the link in the navigator object. "
			"If pressed twice, displays the URL in a browseable message."
		),
		gesture="kb:NVDA+k",
		category=SCRCAT_TOOLS
		)
	def script_speakOrBrowseLinkDestination(self, gesture, forceBrowseable: bool = False) -> None:
	...
	@script(
		description=_(
			...
			"message, instead of just speaking it. May be preferred by braille users."
		),
		category=SCRCAT_TOOLS
		)
	def script_browseLinkDestination(self, gesture) -> None:
		self.script_speakOrBrowseLinkDestination(gesture, True)
