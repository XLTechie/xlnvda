...
class PortableCreaterDialog(
	...
	def onCreatePortable(self, evt):
		...
		if not os.path.isabs(expandedPortableDirectory):
			gui.messageBox(
				_(
					...
					# in the Create Portable NVDA dialog.
					"Please specify an absolute path (including drive letter)  in which to create the "
					"portable copy. It may include system variables (%temp%, %userprofile%, etc.)."
				),
		...
		self.Hide()
		doCreatePortable(
			expandedPortableDirectory, self.copyUserConfigCheckbox.Value,
			False, self.startAfterCreateCheckbox.Value
		)
		self.Destroy()
